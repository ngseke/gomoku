...
#chat
  ul(ref='chatList')
    transition-group(name='chat-item')
      li(v-for='(i, id, index) in chat' :key='id' :title='convertDate(i.date)')
        div(v-if='i.player')
          b [{{ i.player.name }}]
  ...
  .input-area
    ...
    input(type='text' v-model.trim='chatInputText' @keyup.enter='sendChat()' placeholder='' max='100')
</template>
  ...
  methods: {
    sendChat () {
      if(this.chatInputText !== ``)
        db.sendChat(this.roomId, this.chatInputText, this.fingerprint)
      this.chatInputText = ``
    },
